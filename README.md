...
- 🚂 **[Suite Éveil](docs/EVEIL.md)** — deux apps iPad **gratuites, sans pub ni traçage, 100 % sur l'appareil** pour les 3 ans et + : un **petit train des mots** dont les wagons s'allument syllabe par syllabe et dont le fourgon s'accroche quand l'enfant prononce la **fin du mot** (FR/EN), et un atelier de coloriage qui exerce la motricité fine. Détecteur acoustique **explicable** (pas de boîte noire) avec une règle d'or : ne jamais dire « il manque la fin » sans preuve. Référence stdlib ⟶ [69 tests](eveil/reference/wordend/) + banc de validation pour orthophonistes + portage Swift/SwiftUI + [état de l'art sourcé](docs/EVEIL-SOURCES.md). — 📁 [`eveil/`](eveil/)
